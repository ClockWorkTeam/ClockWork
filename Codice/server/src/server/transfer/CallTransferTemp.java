...
public class CallTransferTemp extends ListenerTransfer {
  		...
  		WebSocketPacket wspacket=null;
   		if(type.equals("call")){
    		...
    		sendPacket(wspacket,connector);
  		  	...
  		  	WebSocketConnector connector = getUserConnector(token.getString("contact"));
  		  	if(connector!=null){
  		  		wspacket=new RawPacket("{\"type\":\"callConference\", \"contact\":\""+event.getConnector().getUsername()+"\",\"typecall\":\""+token.getString("calltype")+"\"}");
  		  	}else{
  		  		wspacket=new RawPacket("{\"type\":\"answeredCall\", \"answer\":\"error\"}");
  		  		connector=event.getConnector();
  		  	}
